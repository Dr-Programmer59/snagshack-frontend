...
const mobileGuide = (
  <div className="space-y-4 text-base text-white">
    <p>
      <strong className="font-bold leading">Here’s a quick and simple guide to ordering food using SnagShack’s bot on your phone. Let’s get started!</strong>
    </p>
    <p>
...
const ContentBox = ({ messages, setMessages, inputValue, setinputValue }) => {
  ...
  const fetchData = async (question) => {
    ...
    if (question.trim() == "otp" || question.trim() == "food" || question.trim() == "guide" || question.trim() == "vcc") {
      setMessages(prevMessages => [...prevMessages, { msg: `we see you are tying to use the ${question} command re type the message in all caps`, "role": "bot" }]);
      return
    }
    ...
    if (question.trim() == "FOOD") {
      ...
      if (user?.subscription_plan) {
        ...
        if (email) {
          setMessages([...messages, {
            msg: `🍕 We’re sending your account now. Need help with ordering? Type 'GUIDE' for a quick guide. 🍔 Enjoy your meal!
            ${email}`, "role": "bot"
          }])
      ...
      try {
        ...
        setMessages([...messages, {
          msg: `We 've got your back! 🔐 Your OTP code is  📩
                    ${otp}`, "role": "bot"
        }])
        setcurrentEmail("")
      }
  ...
  return (
    <>
      ...
      <div className="h-screen-[100px] flex flex-col hide-scrollbar">
        <div className="flex-grow overflow-hidden hide-scrollbar scroll-smooth pb-5">
          <div className="h-full overflow-y-auto pb-24 hide-scrollbar">
            <div className='w-full max-w-[1040px] mx-auto flex flex-col py-[50px] lg:pt-[10px] justify-center items-center md:px-[10px] hide-scrollbar md:py-0'>
              {
                user ? <>
                </>
                  :
                  <>
                    <Image src={Logo} alt="logo" className="h-[154px] w-[154px] lg:h-[130px] lg:w-[130px]" />

                    <div className="flex flex-col gap-y-5 lg:gap-y-3 items-center px-[20px]">
                      <Heading text="Meet the Snag Bot " />
                      <Text content="Snag A Deal At The Shack!" customClass="text-center lg:text-[14px]" />

                      <div className="flex flex-wrap gap-5 justify-center ">
                        <Button name="Create Account" customClass="h-[49px] lg:h-[40px] w-[265px] lg:w-[210px] bg-primary text-black font-bold"
                        ...
                        />
                        <Button name="Login" customClass="h-[49px] lg:h-[40px] w-[265px] lg:w-[210px] bg-black text-white font-light border border-white/30"
              ...
              <div className="w-full hide-scrollbar">
                <div className="w-full flex flex-col gap-y-2 md:gap-y-4 lg:px-[20px] ">
                  {
                    messages.map((value) => (
